...
export const BingChatConfig: PlatformConfig = {
  name: 'BingChat',
  url: 'https://www.bing.com/chat',
  authRequired: true,
  edgeOnly: true, // Bing Chat only works in Edge
...
export interface BingChatConversation {
  id: string;
  title: string;
  messages: BingChatMessage[];
  createdAt: number;
  updatedAt: number;
  sources?: string[];
}

export class BingChatPlatform {
  private config: PlatformConfig;

  constructor() {
    this.config = BingChatConfig;
  }
   ...
   */
  async getConversations(page: any): Promise<BingChatConversation[]> {
    try {
   ...
   */
  async getCurrentMessages(page: any): Promise<BingChatMessage[]> {
    try {
      const messages = await page.evaluate(() => {
        ...
        return Array.from(messageElements).map((element, index) => {
          ...
          const sourcesElement = element.querySelector('[data-testid="message-sources"]');
          
          const role = roleElement?.textContent?.toLowerCase() === 'user' ? 'user' : 'assistant';
          const content = contentElement?.textContent || '';
          const sources = sourcesElement ? Array.from(sourcesElement.querySelectorAll('a')).map(a => a.href) : [];
   ...
   */
  async getConversationSources(page: any): Promise<string[]> {
    try {
      const sources = await page.evaluate(() => {
        const sourceElements = document.querySelectorAll('[data-testid="source-link"]');
        return Array.from(sourceElements).map(element => {
   ...
   */
  async setConversationTone(page: any, tone: 'creative' | 'balanced' | 'precise'): Promise<void> {
    try {
      const toneSelector = await page.$('[data-testid="tone-selector"]');
      if (toneSelector) {
        await toneSelector.click();
        
        const toneOption = await page.$(`[data-testid="tone-option-${tone}"]`);
        if (toneOption) {
          await toneOption.click();
        }
      }
    } catch (error) {
      console.error('Failed to set conversation tone:', error);
    }
  }
   ...
   */
  async getConversationTone(page: any): Promise<string | null> {
    try {
      const tone = await page.evaluate(() => {
        const toneElement = document.querySelector('[data-testid="current-tone"]');
        return toneElement?.textContent || null;
      });
      
      return tone;
    } catch (error) {
      console.error('Failed to get conversation tone:', error);
      return null;
    }
  }
} 